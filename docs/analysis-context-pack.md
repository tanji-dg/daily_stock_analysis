...
**Scope（本轮实现范围）：**`#1381` 仅覆盖后端大盘上下文注入、当日/目标交易日复用控制与保守护栏；本次变更不包含独立 API、Web 阶段结果独立展示、四阶段日报结构化持久化或新增日报状态表。范围对应目录与入口为 `main.py`（调度与 `--no-market-review`）、`src/core/pipeline.py`、`src/services/daily_market_context.py`、`src/analyzer.py`、`src/agent/*.py`、`src/daily_market_context_guardrail.py`。
