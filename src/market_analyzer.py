...
"""
        ...
        if review_language == "ja":
            report_title = self._get_review_title(overview.date).removeprefix("## ").strip()
            return f"""あなたはプロの市場アナリストです。以下のデータに基づいて、{self._get_market_scope_name('ja')}の簡潔な市場リキャップレポートを作成してください。

【出力要件】
- 純粋な Markdown テキストのみで出力すること
- JSON を出力しないこと
- コードブロックを出力しないこと
- 見出しでの emoji は控えめに（見出し1つにつき最大1個）
- 固定の枠組み・見出し・説明・結論はすべて日本語で記述すること
- システムが注入済みの表データを重複して列挙せず、本文では表の背後にある意味を解説すること
...
# 出力フォーマット（この構成に従って出力すること）

## {report_title}

> 本日の市場状態、中心的な争点、翌営業日に優先して観察すべき方向を一言で示す。

### 1. 全体概況
（指数・値動き・売買代金・心理の温度を2〜3文でまとめ、「強い/やや強い/もみ合い/やや弱い」の判断を明示する）

### 2. 指数構造
（{self._get_index_hint()}。どれが下支えし、どれが重しになっているか、主要な支持線・抵抗線を説明する）

### 3. セクター主力
（上昇・下落の主導セクターの背後にあるロジック、持続性、主力テーマを形成しているかを分析する）

### 4. 資金と心理
（売買代金、市場の広がり、リスク選好を読み解く）

### 5. ニュース・カタリスト
（直近3日のニュースを踏まえ、翌営業日の取引に本当に影響するカタリストや撹乱要因を抽出する）

### 6. 翌営業日の取引計画
（攻め/均衡/守りの結論、ポジション範囲、注目方向、回避方向、失効トリガーを1つ示す）

### 7. リスク警告
（注視すべきリスクを列挙する。最後に「本内容は参考情報であり、投資助言ではありません。」を添える）

---

レポート本文のみを直接出力し、それ以外の説明文は出力しないこと。
"""
