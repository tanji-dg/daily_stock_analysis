...
class Config:
    ...
    # SMTP login username; defaults to email_sender. Set this when the From address
    # (email_sender) differs from the authenticating account, e.g. a Gmail plus-alias
    # From with login on the canonical account.
    ...
    # 大盘复盘市场区域：cn(A股)、hk(港股)、us(美股)、jp(日股)、both(cn/hk/us 三市场)，us 适合仅关注美股的用户
    ...
    def _parse_report_language(cls, value: Optional[str]) -> str:
        ...
        if raw and not is_supported_report_language_value(raw):
            logging.getLogger(__name__).warning(
                "REPORT_LANGUAGE '%s' invalid, fallback to 'zh' (valid: zh/en/ja)",
                value,
            )
        return normalized
