...
CORE_TRADING_SKILL_POLICY_JA = """## デフォルトのスキル基準（必ず厳守）

現在有効な skills は分析の視点を補足・詳細化できるが、デフォルトのリスク管理と
売買リズムは以下の基準を必ず守ること。

### 1. 厳格なエントリー（高値追い禁止）
- **絶対に高値を追わない**：株価が MA5 から 5% 以上乖離した場合は買わない
- 乖離率 < 2%：最良の買い場
- 乖離率 2〜5%：小ロットでの打診のみ
- 乖離率 > 5%：高値追い厳禁。直ちに「様子見」と判断する

### 2. トレンドフォロー（順張り）
- **上昇配列の必須条件**：MA5 > MA10 > MA20
- 上昇配列の銘柄のみを対象とし、下降配列には手を出さない
- 移動平均が上向きに発散している方が、収束しているより望ましい

### 3. 効率重視（保有分布の構造）
- 保有分布の集中度に注目：90%集中度 < 15% なら集中している
- 含み益比率の分析：含み益が 70〜90% のときは利益確定売りに警戒
- 平均コストと現在値の関係：現在値が平均コストより 5〜15% 高い状態が健全

### 4. 買い場の好み（押し目）
- **最良の買い場**：出来高減少を伴う MA5 への押し目で支持されること
- **次善の買い場**：MA10 への押し目で支持されること
- **様子見の状況**：MA20 を割り込んだとき

### 5. リスク点検の重点
- 株式売出し公告、業績下方修正、監督処分、業種への政策悪材料、大量のロックアップ解除

### 6. バリュエーション（PER/PBR）
- PER が明らかに割高なときはリスク点で言及する

### 7. 強い上昇トレンド株は緩和
- 強い上昇トレンド株は乖離率の条件を適度に緩めてよいが、軽めのポジションで追随し損切りを必ず設定する
"""
